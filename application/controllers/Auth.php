...
class Auth extends CI_Controller
{
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'admsimpentra@gmail.com',
            'smtp_pass' => 'haduh123',
            'smtp_port' =>  465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
        ...
        $this->email->from('admsimpentra@gmail.com', 'Administrator Simpentra');
    ...
    public function forgotPassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $email = $this->input->post('email');
            $role_id = $this->input->post('role_id');
            $user = $this->db->get_where('user', ['email' => $email, 'role_id' => $role_id, 'is_active' => 1])->row_array();

            if ($user) {
                $token = base64_encode(random_bytes(32));
                $user_token = [
                    'token' => $token,
                    'date_created_token' => date("Y-m-d", time())
                ];
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Please check your email <b>(maybe on spam)</b> for reset your password!</div>');
